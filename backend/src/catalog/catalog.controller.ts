...
export class CatalogController {
  ...
  @Get('reasons')
  listReasons(@Query('type') type?: ReasonType) {
    return this.catalogService.listReasons(type);
  }

  @Post('reasons')
  @Roles(UserRole.ADMIN, UserRole.ADMINISTRATIVE, UserRole.MANAGER)
  createReason(@Body() dto: CreateReasonDto, @Req() req: { user: { id: string } }) {
    return this.catalogService.createReason(dto, req.user.id);
  }
}
